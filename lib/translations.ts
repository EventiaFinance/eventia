export type Locale = 'fr' | 'en' | 'de' | 'es' | 'pt'

export const locales: { code: Locale; label: string; flag: string }[] = [
  { code: 'fr', label: 'Français', flag: '🇫🇷' },
  { code: 'en', label: 'English', flag: '🇬🇧' },
  { code: 'de', label: 'Deutsch', flag: '🇩🇪' },
  { code: 'es', label: 'Español', flag: '🇪🇸' },
  { code: 'pt', label: 'Português', flag: '🇵🇹' },
]

export const translations = {
  fr: {
    nav: {
      home: 'Accueil',
      personal: 'Prêts Personnels',
      mortgage: 'Prêts Immobiliers',
      rachat: 'Rachat de Crédit',
      travaux: 'Crédit Travaux',
      assurances: 'Assurances',
      about: 'À Propos',
      apply: 'Faire une demande de prêt',
    },
    hero: {
      welcome: 'Bienvenue chez EVENTIA FINANCE',
      subtitle: 'Votre spécialiste du financement',
      description:
        'Découvrez nos services de financement avec un accompagnement expert et personnalisé.',
      cta1: 'Faire une demande',
      cta2: 'Simuler mon prêt',
      legal:
        'Un crédit vous engage et doit être remboursé. Vérifiez vos capacités de remboursement avant de vous engager.',
    },
    services: {
      title: 'Nos Services de Financement',
      subtitle: 'Des solutions adaptées à tous vos besoins de financement',
      personal: {
        title: 'Prêts Personnels',
        desc: 'Financez vos projets personnels avec des conditions avantageuses',
      },
      mortgage: {
        title: 'Prêts Immobiliers',
        desc: 'Concrétisez votre projet immobilier avec notre expertise',
      },
      consolidation: {
        title: 'Rachat de Crédit',
        desc: 'Simplifiez vos remboursements et diminuez vos mensualités',
      },
      travaux: {
        title: 'Crédit Travaux',
        desc: 'Financez vos projets de rénovation et d\'aménagement de votre logement',
      },
      assurances: {
        title: 'Assurances',
        desc: 'Protégez vos proches et vos biens avec nos solutions sur mesure',
      },
      discover: 'Découvrir',
    },
    why: {
      title: 'Pourquoi Choisir EVENTIA FINANCE',
      security: {
        title: 'Sécurité & Confiance',
        desc: "Établissement agréé avec plus de 19 ans d'expérience en financement",
      },
      speed: {
        title: 'Rapidité & Efficacité',
        desc: 'Réponse sous 48h et déblocage rapide de vos fonds',
      },
      support: {
        title: 'Accompagnement Expert',
        desc: '43 conseillers spécialisés pour vous guider à chaque étape',
      },
    },
    simulator: {
      title: 'Simulateur de prêt',
      subtitle: 'Calculez votre financement en quelques étapes',
      params: 'Paramètres du prêt',
      loanType: 'Type de prêt',
      selectType: 'Sélectionner un type',
      amount: 'Montant',
      duration: 'Durée',
      months: 'mois',
      years: 'ans',
      results: 'Résultats',
      monthly: 'Mensualité estimée',
      totalCost: 'Coût total du crédit',
      totalAmount: 'Montant total dû',
      rate: 'Taux indicatif (TAEG)',
      resultNote:
        'Simulation indicative, sans valeur contractuelle. Contactez-nous pour une offre personnalisée.',
      cta: 'Faire ma demande',
      types: {
        personal: 'Prêt personnel',
        mortgage: 'Prêt immobilier',
        consolidation: 'Rachat de crédit',
        auto: 'Crédit auto',
      },
    },
    testimonials: {
      title: 'Témoignages clients',
      subtitle: 'Ce que disent nos clients',
      reviews: 'avis',
      loanType: 'Type de prêt',
      amount: 'Montant',
      prev: 'Précédent',
      next: 'Suivant',
      items: [
        {
          name: 'Marie Dubois',
          city: 'Paris',
          text: 'Excellent service, rapide et professionnel. Je recommande EVENTIA FINANCE !',
          type: 'Prêt personnel',
          amount: '25 000 €',
        },
        {
          name: 'Pierre Martin',
          city: 'Lyon',
          text: "Grâce à EVENTIA FINANCE, j'ai obtenu mon prêt immobilier aux meilleures conditions.",
          type: 'Prêt immobilier',
          amount: '250 000 €',
        },
        {
          name: 'Sophie Leroux',
          city: 'Marseille',
          text: 'Parfait pour regrouper mes crédits, beaucoup plus simple maintenant !',
          type: 'Rachat de crédit',
          amount: '45 000 €',
        },
      ],
    },
    guarantees: {
      title: 'Nos garanties',
      satisfied: 'Clients satisfaits',
      response: 'Délai de réponse moyen',
      rating: 'Note moyenne',
    },
    cta: {
      title: 'Prêt à Financer Votre Projet ?',
      subtitle: 'Nos experts vous accompagnent dans votre projet de financement',
      button: 'Commencer ma demande',
    },
    form: {
      pageTitle: 'Demande de Crédit',
      pageSubtitle:
        "Complétez votre demande de financement en quelques minutes. Notre équipe d'experts vous contactera rapidement pour étudier votre projet et vous proposer les meilleures solutions adaptées à votre profil.",
      formTitle: 'Formulaire de Demande de Crédit',
      personalInfo: 'Informations Personnelles',
      firstName: 'Prénom',
      lastName: 'Nom de famille',
      email: 'Adresse e-mail',
      confirmEmail: "Confirmer l'adresse e-mail",
      phone: 'Téléphone',
      project: 'Votre Projet de Financement',
      creditType: 'Type de crédit',
      chooseType: 'Choisissez le type de crédit',
      currency: 'Devise',
      chooseCurrency: 'Choisissez la devise',
      currencies: {
        EUR: 'Euro (€)',
        USD: 'Dollar américain ($)',
        GBP: 'Livre sterling (£)',
        CHF: 'Franc suisse (CHF)',
        CAD: 'Dollar canadien (C$)',
        AUD: 'Dollar australien (A$)',
        JPY: 'Yen japonais (¥)',
        CNY: 'Yuan chinois (元)',
        INR: 'Roupie indienne (₹)',
        BRL: 'Réal brésilien (R$)',
      },
      desiredAmount: 'Montant souhaité',
      desiredDuration: 'Durée souhaitée (en mois)',
      situation: 'Votre Situation Financière',
      profession: 'Situation professionnelle',
      selectSituation: 'Sélectionnez votre situation',
      professions: {
        employee: 'Salarié(e)',
        selfEmployed: 'Indépendant(e)',
        civilServant: 'Fonctionnaire',
        retired: 'Retraité(e)',
        other: 'Autre',
      },
      income: 'Revenus mensuels nets',
      message: 'Message complémentaire',
      submit: 'Envoyer ma demande de crédit',
      required: '* Champs obligatoires',
      trust: 'Pourquoi nous faire confiance ?',
      secure: 'Données sécurisées',
      secureDesc: 'Vos informations sont protégées et traitées en toute confidentialité',
      fast: 'Réponse rapide',
      fastDesc: 'Nous vous recontactons sous 24h maximum',
      advisors: 'Conseillers disponibles',
      advisorsDesc: '43 experts à votre service du lundi au vendredi',
      advantages: 'Nos Avantages',
      advantagesList: [
        "19 ans d'expérience",
        '43 conseillers experts',
        'Taux compétitifs',
        'Accompagnement personnalisé',
      ],
      question: 'Une question ?',
      questionDesc: "N'hésitez pas à nous contacter",
      hours: 'Du lundi au vendredi de 9h à 18h',
      success: 'Votre demande a bien été envoyée ! Un conseiller vous contactera sous 24h.',
      emailMismatch: 'Les adresses e-mail ne correspondent pas.',
    },
    footer: {
      tagline: 'Votre partenaire de confiance pour tous vos projets de financement.',
      navigation: 'Navigation',
      services: 'Services',
      legalInfo: 'Informations Société',
      legal: {
        form: 'Forme juridique : SARL',
        capital: 'Capital : 17 000 000 €',
        siren: 'SIREN : 494 831 191',
        siret: 'SIRET : 494 831 191 00056',
        tva: 'TVA : FR56 494831191',
        rcs: 'RCS Paris n° 424 264 281',
        rne: 'RNE : 19/03/2007',
        staff: 'Personnel : 43 salariés',
      },
      address: '55 Place Charles de Gaulle, 59500 Douai, France',
      rights: 'Tous droits réservés.',
      privacyPolicy: 'Politique de Confidentialité',
      legalNotices: 'Mentions Légales',
      warning:
        'Un crédit vous engage et doit être remboursé. Vérifiez vos capacités de remboursement avant de vous engager.',
    },
    whatsapp: 'Discuter sur WhatsApp',
    pages: {
      personal: {
        title: 'Prêts Personnels',
        subtitle: 'Financez tous vos projets personnels',
        desc: 'Le prêt personnel est un crédit à la consommation sans justificatif d\'utilisation. Vous disposez librement de la somme empruntée pour financer tous vos projets de vie (auto, voyages, événements, équipements).',
        advantagesTitle: 'Avantages des prêts personnels',
        advantages: [
          'Utilisation libre des fonds sans justificatif',
          'Réponse rapide et étude de dossier sous 48h',
          'Taux compétitifs et mensualités fixes',
          'Sans garantie ni caution requise',
          'Accompagnement personnalisé tout au long du prêt',
          'Étude gratuite et sans engagement de votre dossier',
        ]
      },
      mortgage: {
        title: 'Prêts Immobiliers',
        subtitle: 'Réalisez votre projet immobilier',
        desc: 'Devenez propriétaire dans les meilleures conditions. Que ce soit pour l\'achat d\'une résidence principale, secondaire ou un investissement locatif, nos experts négocient pour vous le meilleur taux.',
        advantagesTitle: 'Avantages de nos prêts immobiliers',
        advantages: [
          'Taux fixes et variables extrêmement compétitifs',
          'Financement flexible adapté à vos revenus',
          'Accompagnement complet de la simulation à la signature chez le notaire',
          'Négociation des meilleures conditions de crédit auprès de nos partenaires',
          'Solutions d\'assurance emprunteur sur mesure',
          'Étude gratuite de votre dossier de financement',
        ],
        featuresTitle: 'Caractéristiques des prêts immobiliers',
        features: [
          { title: 'Montants élevés', desc: 'Financement allant jusqu\'à 80% de la valeur du bien' },
          { title: 'Tous projets', desc: 'Achat dans le neuf, l\'ancien, construction ou rénovation' },
          { title: 'Sécurisé', desc: 'Garanties adaptées pour sécuriser votre investissement' },
          { title: 'Taux avantageux', desc: 'Les meilleurs taux négociés du marché' },
        ]
      },
      rachat: {
        title: 'Rachat de Crédit',
        subtitle: 'Regroupez tous vos crédits',
        desc: 'Le rachat de crédit consiste à rassembler tous vos prêts en cours (immobilier, conso, renouvelable) en un seul crédit. Vous réduisez ainsi vos mensualités et simplifiez la gestion de votre budget.',
        advantagesTitle: 'Avantages du rachat de crédit',
        advantages: [
          'Diminuez vos mensualités globales jusqu\'à 60%',
          'Simplifiez votre gestion avec un seul prélèvement par mois',
          'Bénéficiez d\'un taux d\'intérêt unique et potentiellement meilleur',
          'Une seule mensualité fixe et prévisible',
          'Possibilité d\'inclure une trésorerie supplémentaire pour un nouveau projet',
          'Conseil personnalisé et étude gratuite',
        ]
      },
      travaux: {
        title: 'Crédit Travaux',
        subtitle: 'Financez vos projets de rénovation',
        desc: 'Le crédit travaux vous permet de financer tous vos projets d\'aménagement, de rénovation ou de décoration de votre logement. Qu\'il s\'agisse de travaux d\'économie d\'énergie ou d\'agrandissement, nous avons la solution.',
        advantagesTitle: 'Avantages du crédit travaux',
        advantages: [
          'Financement de 100% de vos devis de travaux',
          'Taux préférentiels pour les travaux d\'économie d\'énergie',
          'Durée de remboursement flexible et adaptée à votre budget',
          'Déblocage des fonds simple et rapide sur présentation des devis',
          'Aucun apport personnel requis',
          'Étude et conseils d\'experts gratuits',
        ]
      },
      assurances: {
        title: 'Nos Assurances',
        subtitle: 'Protégez ce qui compte le plus pour vous',
        desc: 'Pour sécuriser vos financements et protéger vos proches, nous vous proposons une gamme complète de contrats d\'assurance adaptés à vos besoins et au meilleur prix du marché.',
        advantagesTitle: 'Nos solutions de protection',
        advantages: [
          'Assurance emprunteur : sécurisez votre prêt en cas de coup dur (décès, invalidité)',
          'Assurance habitation : protégez votre patrimoine immobilier et vos biens',
          'Complémentaire santé : des garanties adaptées pour couvrir vos dépenses médicales',
          'Tarifs négociés parmi les plus bas du marché',
          'Changement d\'assurance emprunteur simple et rapide',
          'Conseillers dédiés pour la gestion de vos contrats',
        ]
      },
      about: {
        title: 'À Propos de Nous',
        subtitle: 'Votre partenaire financier de confiance',
        desc: 'Depuis notre création, notre mission est de vous accompagner dans la réalisation de tous vos projets en vous apportant les solutions de financement les plus adaptées et les plus performantes.',
        advantagesTitle: 'Pourquoi nous faire confiance',
        advantages: [
          'Plus de 19 ans d\'expérience dans le domaine du crédit et du courtage',
          'Une équipe de 43 conseillers experts et à votre écoute',
          'Une totale indépendance vis-à-vis des banques pour défendre vos intérêts',
          'Plus de 10 000 clients satisfaits et accompagnés avec succès',
          'Des valeurs de transparence, de proximité et d\'efficacité',
          'Un service client de qualité récompensé pour sa réactivité',
        ]
      }
    },
  },
  en: {
    nav: {
      home: 'Home',
      personal: 'Personal Loans',
      mortgage: 'Mortgage Loans',
      rachat: 'Debt Consolidation',
      travaux: 'Renovation Loans',
      assurances: 'Insurance',
      about: 'About Us',
      apply: 'Apply for a loan',
    },
    hero: {
      welcome: 'Welcome to EVENTIA FINANCE',
      subtitle: 'Your financing specialist',
      description:
        'Discover our financing services with expert and personalized support.',
      cta1: 'Apply now',
      cta2: 'Simulate my loan',
      legal:
        'A loan is a commitment and must be repaid. Check your repayment capacity before committing.',
    },
    services: {
      title: 'Our Financing Services',
      subtitle: 'Solutions adapted to all your financing needs',
      personal: {
        title: 'Personal Loans',
        desc: 'Finance your personal projects with advantageous conditions',
      },
      mortgage: {
        title: 'Mortgage Loans',
        desc: 'Make your real estate project a reality with our expertise',
      },
      consolidation: {
        title: 'Debt Consolidation',
        desc: 'Simplify your repayments and reduce your monthly payments',
      },
      travaux: {
        title: 'Renovation Loans',
        desc: 'Finance your renovation and home improvement projects',
      },
      assurances: {
        title: 'Insurance',
        desc: 'Protect your loved ones and your assets with our custom insurance solutions',
      },
      discover: 'Discover',
    },
    why: {
      title: 'Why Choose EVENTIA FINANCE',
      security: {
        title: 'Security & Trust',
        desc: 'Licensed institution with over 19 years of financing experience',
      },
      speed: {
        title: 'Speed & Efficiency',
        desc: 'Response within 48h and fast release of your funds',
      },
      support: {
        title: 'Expert Support',
        desc: '43 specialized advisors to guide you every step of the way',
      },
    },
    simulator: {
      title: 'Loan Simulator',
      subtitle: 'Calculate your financing in a few steps',
      params: 'Loan parameters',
      loanType: 'Loan type',
      selectType: 'Select a type',
      amount: 'Amount',
      duration: 'Duration',
      months: 'months',
      years: 'years',
      results: 'Results',
      monthly: 'Estimated monthly payment',
      totalCost: 'Total cost of credit',
      totalAmount: 'Total amount due',
      rate: 'Indicative rate (APR)',
      resultNote:
        'Indicative simulation, non-contractual. Contact us for a personalized offer.',
      cta: 'Apply now',
      types: {
        personal: 'Personal loan',
        mortgage: 'Mortgage loan',
        consolidation: 'Debt consolidation',
        auto: 'Car loan',
      },
    },
    testimonials: {
      title: 'Client Testimonials',
      subtitle: 'What our clients say',
      reviews: 'reviews',
      loanType: 'Loan type',
      amount: 'Amount',
      prev: 'Previous',
      next: 'Next',
      items: [
        {
          name: 'Marie Dubois',
          city: 'Paris',
          text: 'Excellent service, fast and professional. I recommend EVENTIA FINANCE!',
          type: 'Personal loan',
          amount: '€25,000',
        },
        {
          name: 'Pierre Martin',
          city: 'Lyon',
          text: 'Thanks to EVENTIA FINANCE, I got my mortgage at the best conditions.',
          type: 'Mortgage loan',
          amount: '€250,000',
        },
        {
          name: 'Sophie Leroux',
          city: 'Marseille',
          text: 'Perfect for consolidating my loans, much simpler now!',
          type: 'Debt consolidation',
          amount: '€45,000',
        },
      ],
    },
    guarantees: {
      title: 'Our guarantees',
      satisfied: 'Satisfied clients',
      response: 'Average response time',
      rating: 'Average rating',
    },
    cta: {
      title: 'Ready to Finance Your Project?',
      subtitle: 'Our experts support you with your financing project',
      button: 'Start my request',
    },
    form: {
      pageTitle: 'Credit Request',
      pageSubtitle:
        'Complete your financing request in a few minutes. Our team of experts will contact you quickly to study your project and offer you the best solutions adapted to your profile.',
      formTitle: 'Credit Request Form',
      personalInfo: 'Personal Information',
      firstName: 'First name',
      lastName: 'Last name',
      email: 'Email address',
      confirmEmail: 'Confirm email address',
      phone: 'Phone',
      project: 'Your Financing Project',
      creditType: 'Credit type',
      chooseType: 'Choose the credit type',
      currency: 'Currency',
      chooseCurrency: 'Choose currency',
      currencies: {
        EUR: 'Euro (€)',
        USD: 'US Dollar ($)',
        GBP: 'British Pound (£)',
        CHF: 'Swiss Franc (CHF)',
        CAD: 'Canadian Dollar (C$)',
        AUD: 'Australian Dollar (A$)',
        JPY: 'Japanese Yen (¥)',
        CNY: 'Chinese Yuan (元)',
        INR: 'Indian Rupee (₹)',
        BRL: 'Brazilian Real (R$)',
      },
      desiredAmount: 'Desired amount',
      desiredDuration: 'Desired duration (in months)',
      situation: 'Your Financial Situation',
      profession: 'Professional situation',
      selectSituation: 'Select your situation',
      professions: {
        employee: 'Employee',
        selfEmployed: 'Self-employed',
        civilServant: 'Civil servant',
        retired: 'Retired',
        other: 'Other',
      },
      income: 'Net monthly income',
      message: 'Additional message',
      submit: 'Send my credit request',
      required: '* Required fields',
      trust: 'Why trust us?',
      secure: 'Secure data',
      secureDesc: 'Your information is protected and treated in complete confidentiality',
      fast: 'Fast response',
      fastDesc: 'We will contact you within 24h maximum',
      advisors: 'Available advisors',
      advisorsDesc: '43 experts at your service Monday to Friday',
      advantages: 'Our Advantages',
      advantagesList: [
        '19 years of experience',
        '43 expert advisors',
        'Competitive rates',
        'Personalized support',
      ],
      question: 'A question?',
      questionDesc: 'Do not hesitate to contact us',
      hours: 'Monday to Friday from 9am to 6pm',
      success: 'Your request has been sent! An advisor will contact you within 24h.',
      emailMismatch: 'Email addresses do not match.',
    },
    footer: {
      tagline: 'Your trusted partner for all your financing projects.',
      navigation: 'Navigation',
      services: 'Services',
      legalInfo: 'Company Information',
      legal: {
        form: 'Legal form: SARL',
        capital: 'Capital: €17,000,000',
        siren: 'SIREN: 494 831 191',
        siret: 'SIRET: 494 831 191 00056',
        tva: 'VAT: FR56 494831191',
        rcs: 'RCS Paris n° 424 264 281',
        rne: 'RNE: 19/03/2007',
        staff: 'Staff: 43 employees',
      },
      address: '55 Place Charles de Gaulle, 59500 Douai, France',
      rights: 'All rights reserved.',
      privacyPolicy: 'Privacy Policy',
      legalNotices: 'Legal Notices',
      warning:
        'A loan is a commitment and must be repaid. Check your repayment capacity before committing.',
    },
    whatsapp: 'Chat on WhatsApp',
    pages: {
      personal: {
        title: 'Personal Loans',
        subtitle: 'Finance all your personal projects',
        desc: 'A personal loan is a consumer credit without proof of usage. You are free to use the borrowed sum to finance all your life projects (car, travel, events, equipment).',
        advantagesTitle: 'Advantages of personal loans',
        advantages: [
          'Free use of funds without proof of expense',
          'Fast response and file review within 48 hours',
          'Competitive rates and fixed monthly payments',
          'No collateral or guarantor required',
          'Personalized support throughout the loan',
          'Free and non-binding study of your application',
        ]
      },
      mortgage: {
        title: 'Mortgage Loans',
        subtitle: 'Realize your real estate project',
        desc: 'Become a homeowner under the best conditions. Whether for the purchase of a primary residence, a secondary residence, or a rental investment, our experts negotiate the best rate for you.',
        advantagesTitle: 'Advantages of our mortgage loans',
        advantages: [
          'Extremely competitive fixed and variable rates',
          'Flexible financing tailored to your income',
          'Complete support from simulation to signing at the notary',
          'Negotiation of the best credit conditions with our partners',
          'Tailor-made borrower insurance solutions',
          'Free study of your financing file',
        ],
        featuresTitle: 'Characteristics of mortgage loans',
        features: [
          { title: 'High amounts', desc: 'Financing up to 80% of the property value' },
          { title: 'All projects', desc: 'Purchase of new, old, construction or renovation properties' },
          { title: 'Secure', desc: 'Suitable guarantees to secure your investment' },
          { title: 'Advantageous rates', desc: 'The best negotiated market rates' },
        ]
      },
      rachat: {
        title: 'Debt Consolidation',
        subtitle: 'Consolidate all your credits',
        desc: 'Debt consolidation (or credit regrouping) consists of gathering all your active loans (mortgage, consumer, revolving) into a single credit. You reduce your monthly payments and simplify your budget management.',
        advantagesTitle: 'Advantages of debt consolidation',
        advantages: [
          'Reduce your overall monthly payments by up to 60%',
          'Simplify your management with a single debit per month',
          'Benefit from a single and potentially better interest rate',
          'A single fixed and predictable monthly payment',
          'Possibility to include extra cash for a new project',
          'Personalized advice and free study',
        ]
      },
      travaux: {
        title: 'Renovation Loans',
        subtitle: 'Finance your renovation projects',
        desc: 'A renovation loan allows you to finance all layout, renovation, or decoration projects for your home. Whether it is energy saving or home extension work, we have the solution.',
        advantagesTitle: 'Advantages of renovation loans',
        advantages: [
          'Financing of 100% of your work estimates',
          'Preferential rates for energy-saving works',
          'Flexible repayment period adapted to your budget',
          'Simple and quick release of funds on presentation of estimates',
          'No personal contribution required',
          'Free expert study and advice',
        ]
      },
      assurances: {
        title: 'Our Insurance',
        subtitle: 'Protect what matters most to you',
        desc: 'To secure your financing and protect your loved ones, we offer a comprehensive range of insurance policies tailored to your needs at the best market prices.',
        advantagesTitle: 'Our protection solutions',
        advantages: [
          'Borrower insurance: secure your loan in case of hard times (death, disability)',
          'Home insurance: protect your real estate assets and belongings',
          'Health insurance: adapted guarantees to cover your medical expenses',
          'Negotiated rates among the lowest in the market',
          'Simple and fast borrower insurance transfer',
          'Dedicated advisors for managing your contracts',
        ]
      },
      about: {
        title: 'About Us',
        subtitle: 'Your trusted financial partner',
        desc: 'Since our creation, our mission has been to support you in realizing all your projects by providing you with the most suitable and efficient financing solutions.',
        advantagesTitle: 'Why trust us',
        advantages: [
          'Over 19 years of experience in the field of credit and brokerage',
          'A team of 43 expert advisors at your service',
          'Complete independence from banks to defend your interests',
          'More than 10,000 satisfied and successfully supported clients',
          'Values of transparency, proximity, and efficiency',
          'A quality client service rewarded for its responsiveness',
        ]
      }
    },
  },
  de: {
    nav: {
      home: 'Startseite',
      personal: 'Privatkredite',
      mortgage: 'Immobilienkredite',
      rachat: 'Kreditumschuldung',
      travaux: 'Renovierungskredite',
      assurances: 'Versicherungen',
      about: 'Über Uns',
      apply: 'Kredit anfragen',
    },
    hero: {
      welcome: 'Willkommen bei EVENTIA FINANCE',
      subtitle: 'Ihr Finanzierungsspezialist',
      description:
        'Entdecken Sie unsere Finanzierungsdienstleistungen mit fachkundiger und persönlicher Betreuung.',
      cta1: 'Antrag stellen',
      cta2: 'Kredit simulieren',
      legal:
        'Ein Kredit ist eine Verpflichtung und muss zurückgezahlt werden. Prüfen Sie Ihre Rückzahlungsfähigkeit, bevor Sie sich verpflichten.',
    },
    services: {
      title: 'Unsere Finanzierungsleistungen',
      subtitle: 'Lösungen für alle Ihre Finanzierungsbedürfnisse',
      personal: {
        title: 'Privatkredite',
        desc: 'Finanzieren Sie Ihre persönlichen Projekte zu vorteilhaften Konditionen',
      },
      mortgage: {
        title: 'Immobilienkredite',
        desc: 'Verwirklichen Sie Ihr Immobilienprojekt mit unserer Expertise',
      },
      consolidation: {
        title: 'Umschuldung',
        desc: 'Vereinfachen Sie Ihre Rückzahlungen und senken Sie Ihre Monatsraten',
      },
      travaux: {
        title: 'Renovierungskredite',
        desc: 'Finanzieren Sie Ihre Renovierungs- und Modernisierungsprojekte',
      },
      assurances: {
        title: 'Versicherungen',
        desc: 'Schützen Sie Ihre Angehörigen und Ihr Eigentum mit unseren maßgeschneiderten Versicherungslösungen',
      },
      discover: 'Entdecken',
    },
    why: {
      title: 'Warum EVENTIA FINANCE wählen',
      security: {
        title: 'Sicherheit & Vertrauen',
        desc: 'Zugelassenes Institut mit über 19 Jahren Finanzierungserfahrung',
      },
      speed: {
        title: 'Schnelligkeit & Effizienz',
        desc: 'Antwort innerhalb von 48 Stunden und schnelle Auszahlung Ihrer Mittel',
      },
      support: {
        title: 'Fachkundige Betreuung',
        desc: '43 spezialisierte Berater begleiten Sie bei jedem Schritt',
      },
    },
    simulator: {
      title: 'Kreditsimulator',
      subtitle: 'Berechnen Sie Ihre Finanzierung in wenigen Schritten',
      params: 'Kreditparameter',
      loanType: 'Kreditart',
      selectType: 'Typ auswählen',
      amount: 'Betrag',
      duration: 'Laufzeit',
      months: 'Monate',
      years: 'Jahre',
      results: 'Ergebnisse',
      monthly: 'Geschätzte Monatsrate',
      totalCost: 'Gesamtkosten des Kredits',
      totalAmount: 'Gesamtbetrag',
      rate: 'Indikativer Zinssatz (effektiver Jahreszins)',
      resultNote:
        'Unverbindliche Simulation. Kontaktieren Sie uns für ein persönliches Angebot.',
      cta: 'Antrag stellen',
      types: {
        personal: 'Privatkredit',
        mortgage: 'Immobilienkredit',
        consolidation: 'Umschuldung',
        auto: 'Autokredit',
      },
    },
    testimonials: {
      title: 'Kundenstimmen',
      subtitle: 'Was unsere Kunden sagen',
      reviews: 'Bewertungen',
      loanType: 'Kreditart',
      amount: 'Betrag',
      prev: 'Zurück',
      next: 'Weiter',
      items: [
        {
          name: 'Marie Dubois',
          city: 'Paris',
          text: 'Ausgezeichneter Service, schnell und professionell. Ich empfehle EVENTIA FINANCE!',
          type: 'Privatkredit',
          amount: '25.000 €',
        },
        {
          name: 'Pierre Martin',
          city: 'Lyon',
          text: 'Dank EVENTIA FINANCE habe ich meinen Immobilienkredit zu besten Konditionen erhalten.',
          type: 'Immobilienkredit',
          amount: '250.000 €',
        },
        {
          name: 'Sophie Leroux',
          city: 'Marseille',
          text: 'Perfekt für die Zusammenlegung meiner Kredite, jetzt viel einfacher!',
          type: 'Umschuldung',
          amount: '45.000 €',
        },
      ],
    },
    guarantees: {
      title: 'Unsere Garantien',
      satisfied: 'Zufriedene Kunden',
      response: 'Durchschnittliche Antwortzeit',
      rating: 'Durchschnittliche Bewertung',
    },
    cta: {
      title: 'Bereit, Ihr Projekt zu finanzieren?',
      subtitle: 'Unsere Experten begleiten Sie bei Ihrem Finanzierungsprojekt',
      button: 'Antrag starten',
    },
    form: {
      pageTitle: 'Kreditantrag',
      pageSubtitle:
        'Füllen Sie Ihren Finanzierungsantrag in wenigen Minuten aus. Unser Expertenteam wird Sie schnell kontaktieren, um Ihr Projekt zu prüfen und Ihnen die besten Lösungen anzubieten.',
      formTitle: 'Kreditantragsformular',
      personalInfo: 'Persönliche Informationen',
      firstName: 'Vorname',
      lastName: 'Nachname',
      email: 'E-Mail-Adresse',
      confirmEmail: 'E-Mail-Adresse bestätigen',
      phone: 'Telefon',
      project: 'Ihr Finanzierungsprojekt',
      creditType: 'Kreditart',
      chooseType: 'Wählen Sie die Kreditart',
      currency: 'Währung',
      chooseCurrency: 'Währung auswählen',
      currencies: {
        EUR: 'Euro (€)',
        USD: 'US-Dollar ($)',
        GBP: 'Britisches Pfund (£)',
        CHF: 'Schweizer Franken (CHF)',
        CAD: 'Kanadischer Dollar (C$)',
        AUD: 'Australischer Dollar (A$)',
        JPY: 'Japanischer Yen (¥)',
        CNY: 'Chinesischer Yuan (元)',
        INR: 'Indische Rupie (₹)',
        BRL: 'Brasilianischer Real (R$)',
      },
      desiredAmount: 'Gewünschter Betrag',
      desiredDuration: 'Gewünschte Laufzeit (in Monaten)',
      situation: 'Ihre finanzielle Situation',
      profession: 'Berufliche Situation',
      selectSituation: 'Wählen Sie Ihre Situation',
      professions: {
        employee: 'Angestellte(r)',
        selfEmployed: 'Selbstständig',
        civilServant: 'Beamte(r)',
        retired: 'Rentner(in)',
        other: 'Sonstiges',
      },
      income: 'Monatliches Nettoeinkommen',
      message: 'Zusätzliche Nachricht',
      submit: 'Kreditantrag senden',
      required: '* Pflichtfelder',
      trust: 'Warum uns vertrauen?',
      secure: 'Sichere Daten',
      secureDesc: 'Ihre Informationen sind geschützt und werden vertraulich behandelt',
      fast: 'Schnelle Antwort',
      fastDesc: 'Wir melden uns innerhalb von maximal 24 Stunden bei Ihnen',
      advisors: 'Verfügbare Berater',
      advisorsDesc: '43 Experten stehen Ihnen von Montag bis Freitag zur Verfügung',
      advantages: 'Unsere Vorteile',
      advantagesList: [
        '19 Jahre Erfahrung',
        '43 Expertenberater',
        'Wettbewerbsfähige Zinsen',
        'Persönliche Betreuung',
      ],
      question: 'Eine Frage?',
      questionDesc: 'Zögern Sie nicht, uns zu kontaktieren',
      hours: 'Montag bis Freitag von 9 bis 18 Uhr',
      success: 'Ihr Antrag wurde gesendet! Ein Berater wird Sie innerhalb von 24 Stunden kontaktieren.',
      emailMismatch: 'Die E-Mail-Adressen stimmen nicht überein.',
    },
    footer: {
      tagline: 'Ihr vertrauenswürdiger Partner für alle Ihre Finanzierungsprojekte.',
      navigation: 'Navigation',
      services: 'Leistungen',
      legalInfo: 'Unternehmensinformationen',
      legal: {
        form: 'Rechtsform: SARL',
        capital: 'Kapital: 17.000.000 €',
        siren: 'SIREN: 494 831 191',
        siret: 'SIRET: 494 831 191 00056',
        tva: 'USt: FR56 494831191',
        rcs: 'RCS Paris n° 424 264 281',
        rne: 'RNE: 19/03/2007',
        staff: 'Mitarbeiter: 43',
      },
      address: '55 Place Charles de Gaulle, 59500 Douai, Frankreich',
      rights: 'Alle Rechte vorbehalten.',
      privacyPolicy: 'Datenschutzerklärung',
      legalNotices: 'Impressum',
      warning:
        'Ein Kredit ist eine Verpflichtung und muss zurückgezahlt werden. Prüfen Sie Ihre Rückzahlungsfähigkeit, bevor Sie sich verpflichten.',
    },
    whatsapp: 'Auf WhatsApp chatten',
    pages: {
      personal: {
        title: 'Privatkredite',
        subtitle: 'Finanzieren Sie alle Ihre persönlichen Projekte',
        desc: 'Ein Privatkredit ist ein Konsumkredit ohne Verwendungsnachweis. Sie können den geliehenen Betrag frei zur Finanzierung all Ihrer Lebensprojekte (Auto, Reisen, Veranstaltungen, Ausstattung) verwenden.',
        advantagesTitle: 'Vorteile von Privatkrediten',
        advantages: [
          'Freie Verwendung der Mittel ohne Verwendungsnachweis',
          'Schnelle Antwort und Prüfung des Dossiers innerhalb von 48 Stunden',
          'Wettbewerbsfähige Zinsen und feste monatliche Raten',
          'Keine Sicherheiten oder Bürgen erforderlich',
          'Persönliche Betreuung während der gesamten Kreditlaufzeit',
          'Kostenlose und unverbindliche Prüfung Ihres Antrags',
        ]
      },
      mortgage: {
        title: 'Immobilienkredite',
        subtitle: 'Realisieren Sie Ihr Immobilienprojekt',
        desc: 'Werden Sie zu besten Konditionen Hauseigentümer. Ob für den Kauf eines Hauptwohnsitzes, eines Zweitwohnsitzes oder eine Mietinvestition – unsere Experten verhandeln für Sie den besten Zinssatz.',
        advantagesTitle: 'Vorteile unserer Immobilienkredite',
        advantages: [
          'Äußerst wettbewerbsfähige feste und variable Zinssätze',
          'Flexible Finanzierung, angepasst an Ihr Einkommen',
          'Komplette Betreuung von der Simulation bis zur Unterzeichnung beim Notar',
          'Verhandlung der besten Kreditkonditionen mit unseren Partnern',
          'Maßgeschneiderte Restschuldversicherungslösungen',
          'Kostenlose Prüfung Ihres Finanzierungsdossiers',
        ],
        featuresTitle: 'Eigenschaften von Immobilienkrediten',
        features: [
          { title: 'Hohe Beträge', desc: 'Finanzierung bis zu 80 % des Immobilienwerts' },
          { title: 'Alle Projekte', desc: 'Kauf von Neubauten, Altbauten, Bau oder Renovierung' },
          { title: 'Sicher', desc: 'Geeignete Garantien zur Absicherung Ihrer Investition' },
          { title: 'Günstige Zinsen', desc: 'Die besten verhandelten Marktzinsen' },
        ]
      },
      rachat: {
        title: 'Kreditumschuldung',
        subtitle: 'Fassen Sie all Ihre Kredite zusammen',
        desc: 'Die Umschuldung (oder Zusammenlegung von Krediten) besteht darin, alle Ihre laufenden Kredite (Immobilien-, Konsum-, revolvierende Kredite) zu einem einzigen Kredit zusammenzufassen. Sie senken Ihre monatlichen Raten und vereinfachen Ihre Budgetverwaltung.',
        advantagesTitle: 'Vorteile der Kreditumschuldung',
        advantages: [
          'Senken Sie Ihre monatlichen Gesamtraten um bis zu 60 %',
          'Vereinfachen Sie Ihre Verwaltung mit nur einer Lastschrift pro Monat',
          'Profitieren Sie von einem einzigen und potenziell besseren Zinssatz',
          'Eine einzige feste und vorhersehbare Monatsrate',
          'Möglichkeit der Einbindung einer zusätzlichen Barreserve für ein neues Projekt',
          'Persönliche Beratung und kostenlose Prüfung',
        ]
      },
      travaux: {
        title: 'Renovierungskredite',
        subtitle: 'Finanzieren Sie Ihre Renovierungsprojekte',
        desc: 'Ein Renovierungskredit ermöglicht Ihnen die Finanzierung aller Einrichtungs-, Renovierungs- oder Dekorationsprojekte für Ihr Zuhause. Ob Energiespar- oder Hauserweiterungsarbeiten – wir haben die Lösung.',
        advantagesTitle: 'Vorteile von Renovierungskrediten',
        advantages: [
          'Finanzierung von 100 % Ihrer Kostenvoranschläge',
          'Vorzugszinsen für Energiesparmaßnahmen',
          'Flexible Rückzahlungsdauer, angepasst an Ihr Budget',
          'Einfache und schnelle Auszahlung der Mittel nach Vorlage der Kostenvoranschläge',
          'Kein Eigenkapital erforderlich',
          'Kostenlose Prüfung und Beratung durch Experten',
        ]
      },
      assurances: {
        title: 'Unsere Versicherungen',
        subtitle: 'Schützen Sie, was Ihnen am wichtigsten ist',
        desc: 'Um Ihre Finanzierungen abzusichern und Ihre Angehörigen zu schützen, bieten wir eine umfassende Palette an Versicherungen an, die auf Ihre Bedürfnisse zugeschnitten sind und zu den besten Marktpreisen angeboten werden.',
        advantagesTitle: 'Unsere Schutzlösungen',
        advantages: [
          'Restschuldversicherung: Sichern Sie Ihren Kredit bei Schicksalsschlägen (Tod, Erwerbsunfähigkeit) ab',
          'Hausratversicherung: Schützen Sie Ihr Immobilienvermögen und Ihren Besitz',
          'Krankenzusatzversicherung: Angepasste Garantien zur Abdeckung Ihrer medizinischen Kosten',
          'Verhandelte Tarife, die zu den niedrigsten am Markt gehören',
          'Einfacher und schneller Wechsel der Restschuldversicherung',
          'Persönliche Berater für die Verwaltung Ihrer Verträge',
        ]
      },
      about: {
        title: 'Über Uns',
        subtitle: 'Ihr vertrauenswürdiger Finanzpartner',
        desc: 'Seit unserer Gründung ist es unsere Mission, Sie bei der Umsetzung all Ihrer Projekte zu begleiten, indem wir Ihnen die am besten geeigneten und effizientesten Finanzierungslösungen anbieten.',
        advantagesTitle: 'Warum Sie uns vertrauen können',
        advantages: [
          'Über 19 Jahre Erfahrung im Bereich Kredit und Vermittlung',
          'Ein Team von 43 erfahrenen Beratern zu Ihren Diensten',
          'Völlige Unabhängigkeit von Banken, um Ihre Interessen zu vertreten',
          'Mehr als 10.000 zufriedene und erfolgreich betreute Kunden',
          'Werte wie Transparenz, Nähe und Effizienz',
          'Ein erstklassiger Kundenservice, der für seine Reaktionsschnelligkeit ausgezeichnet wurde',
        ]
      }
    },
  },
  es: {
    nav: {
      home: 'Inicio',
      personal: 'Préstamos Personales',
      mortgage: 'Préstamos Hipotecarios',
      rachat: 'Reagrupación de Crédito',
      travaux: 'Crédito Reformas',
      assurances: 'Seguros',
      about: 'Sobre Nosotros',
      apply: 'Solicitar un préstamo',
    },
    hero: {
      welcome: 'Bienvenido a EVENTIA FINANCE',
      subtitle: 'Su especialista en financiación',
      description:
        'Descubra nuestros servicios de financiación con un acompañamiento experto y personalizado.',
      cta1: 'Solicitar ahora',
      cta2: 'Simular mi préstamo',
      legal:
        'Un crédito le compromete y debe ser reembolsado. Verifique su capacidad de reembolso antes de comprometerse.',
    },
    services: {
      title: 'Nuestros Servicios de Financiación',
      subtitle: 'Soluciones adaptadas a todas sus necesidades de financiación',
      personal: {
        title: 'Préstamos Personales',
        desc: 'Financie sus proyectos personales con condiciones ventajosas',
      },
      mortgage: {
        title: 'Préstamos Hipotecarios',
        desc: 'Haga realidad su proyecto inmobiliario con nuestra experiencia',
      },
      consolidation: {
        title: 'Reunificación de Deudas',
        desc: 'Simplifique sus reembolsos y reduzca sus cuotas mensuales',
      },
      travaux: {
        title: 'Crédito Reformas',
        desc: 'Financie sus proyectos de reforma y acondicionamiento del hogar',
      },
      assurances: {
        title: 'Seguros',
        desc: 'Proteja a sus seres queridos y sus bienes con nuestras soluciones de seguro a medida',
      },
      discover: 'Descubrir',
    },
    why: {
      title: 'Por Qué Elegir EVENTIA FINANCE',
      security: {
        title: 'Seguridad y Confianza',
        desc: 'Establecimiento autorizado con más de 19 años de experiencia en financiación',
      },
      speed: {
        title: 'Rapidez y Eficacia',
        desc: 'Respuesta en 48h y desbloqueo rápido de sus fondos',
      },
      support: {
        title: 'Acompañamiento Experto',
        desc: '43 asesores especializados para guiarle en cada etapa',
      },
    },
    simulator: {
      title: 'Simulador de préstamo',
      subtitle: 'Calcule su financiación en unos pocos pasos',
      params: 'Parámetros del préstamo',
      loanType: 'Tipo de préstamo',
      selectType: 'Seleccionar un tipo',
      amount: 'Importe',
      duration: 'Duración',
      months: 'meses',
      years: 'años',
      results: 'Resultados',
      monthly: 'Cuota mensual estimada',
      totalCost: 'Coste total del crédito',
      totalAmount: 'Importe total adeudado',
      rate: 'Tasa indicativa (TAE)',
      resultNote:
        'Simulación indicativa, sin valor contractual. Contáctenos para una oferta personalizada.',
      cta: 'Solicitar ahora',
      types: {
        personal: 'Préstamo personal',
        mortgage: 'Préstamo hipotecario',
        consolidation: 'Reunificación de deudas',
        auto: 'Crédito auto',
      },
    },
    testimonials: {
      title: 'Testimonios de clientes',
      subtitle: 'Lo que dicen nuestros clientes',
      reviews: 'opiniones',
      loanType: 'Tipo de préstamo',
      amount: 'Importe',
      prev: 'Anterior',
      next: 'Siguiente',
      items: [
        {
          name: 'Marie Dubois',
          city: 'París',
          text: 'Excelente servicio, rápido y profesional. ¡Recomiendo EVENTIA FINANCE!',
          type: 'Préstamo personal',
          amount: '25.000 €',
        },
        {
          name: 'Pierre Martin',
          city: 'Lyon',
          text: 'Gracias a EVENTIA FINANCE, obtuve mi préstamo hipotecario en las mejores condiciones.',
          type: 'Préstamo hipotecario',
          amount: '250.000 €',
        },
        {
          name: 'Sophie Leroux',
          city: 'Marsella',
          text: 'Perfecto para reunificar mis créditos, ¡mucho más simple ahora!',
          type: 'Reunificación de deudas',
          amount: '45.000 €',
        },
      ],
    },
    guarantees: {
      title: 'Nuestras garantías',
      satisfied: 'Clientes satisfechos',
      response: 'Tiempo medio de respuesta',
      rating: 'Valoración media',
    },
    cta: {
      title: '¿Listo para Financiar su Proyecto?',
      subtitle: 'Nuestros expertos le acompañan en su proyecto de financiación',
      button: 'Comenzar mi solicitud',
    },
    form: {
      pageTitle: 'Solicitud de Crédito',
      pageSubtitle:
        'Complete su solicitud de financiación en unos minutos. Nuestro equipo de expertos le contactará rápidamente para estudiar su proyecto y ofrecerle las mejores soluciones adaptadas a su perfil.',
      formTitle: 'Formulario de Solicitud de Crédito',
      personalInfo: 'Información Personal',
      firstName: 'Nombre',
      lastName: 'Apellidos',
      email: 'Correo electrónico',
      confirmEmail: 'Confirmar correo electrónico',
      phone: 'Teléfono',
      project: 'Su Proyecto de Financiación',
      creditType: 'Tipo de crédito',
      chooseType: 'Elija el tipo de crédito',
      currency: 'Moneda',
      chooseCurrency: 'Elija la moneda',
      currencies: {
        EUR: 'Euro (€)',
        USD: 'Dólar estadounidense ($)',
        GBP: 'Libra esterlina (£)',
        CHF: 'Franco suizo (CHF)',
        CAD: 'Dólar canadiense (C$)',
        AUD: 'Dólar australiano (A$)',
        JPY: 'Yen japonés (¥)',
        CNY: 'Yuan chino (元)',
        INR: 'Rupia india (₹)',
        BRL: 'Real brasileño (R$)',
      },
      desiredAmount: 'Importe deseado',
      desiredDuration: 'Duración deseada (en meses)',
      situation: 'Su Situación Financiera',
      profession: 'Situación profesional',
      selectSituation: 'Seleccione su situación',
      professions: {
        employee: 'Asalariado/a',
        selfEmployed: 'Autónomo/a',
        civilServant: 'Funcionario/a',
        retired: 'Jubilado/a',
        other: 'Otro',
      },
      income: 'Ingresos mensuales netos',
      message: 'Mensaje adicional',
      submit: 'Enviar mi solicitud de crédito',
      required: '* Campos obligatorios',
      trust: '¿Por qué confiar en nosotros?',
      secure: 'Datos seguros',
      secureDesc: 'Su información está protegida y tratada con total confidencialidad',
      fast: 'Respuesta rápida',
      fastDesc: 'Le contactaremos en un máximo de 24h',
      advisors: 'Asesores disponibles',
      advisorsDesc: '43 expertos a su servicio de lunes a viernes',
      advantages: 'Nuestras Ventajas',
      advantagesList: [
        '19 años de experiencia',
        '43 asesores expertos',
        'Tasas competitivas',
        'Acompañamiento personalizado',
      ],
      question: '¿Alguna pregunta?',
      questionDesc: 'No dude en contactarnos',
      hours: 'De lunes a viernes de 9h a 18h',
      success: '¡Su solicitud ha sido enviada! Un asesor le contactará en 24h.',
      emailMismatch: 'Las direcciones de correo no coinciden.',
    },
    footer: {
      tagline: 'Su socio de confianza para todos sus proyectos de financiación.',
      navigation: 'Navegación',
      services: 'Servicios',
      legalInfo: 'Información de la Empresa',
      legal: {
        form: 'Forma jurídica: SARL',
        capital: 'Capital: 17.000.000 €',
        siren: 'SIREN: 494 831 191',
        siret: 'SIRET: 494 831 191 00056',
        tva: 'IVA: FR56 494831191',
        rcs: 'RCS París n° 424 264 281',
        rne: 'RNE: 19/03/2007',
        staff: 'Personal: 43 empleados',
      },
      address: '55 Place Charles de Gaulle, 59500 Douai, Francia',
      rights: 'Todos los derechos reservados.',
      privacyPolicy: 'Política de Privacidad',
      legalNotices: 'Aviso Legal',
      warning:
        'Un crédito le compromete y debe ser reembolsado. Verifique su capacidad de reembolso antes de comprometerse.',
    },
    whatsapp: 'Chatear por WhatsApp',
    pages: {
      personal: {
        title: 'Préstamos Personales',
        subtitle: 'Financie todos sus proyectos personales',
        desc: 'El préstamo personal es un crédito de consumo sin justificante de finalidad. Dispone libremente del importe prestado para financiar todos sus proyectos de vida (coche, viajes, eventos, equipamiento).',
        advantagesTitle: 'Ventajas de los préstamos personales',
        advantages: [
          'Libre uso de los fondos sin justificar',
          'Respuesta rápida y estudio del expediente en 48h',
          'Tasas competitivas y cuotas mensuales fijas',
          'Sin necesidad de garantías ni avales',
          'Acompañamiento personalizado a lo largo del préstamo',
          'Estudio gratuito y sin compromiso de su expediente',
        ]
      },
      mortgage: {
        title: 'Préstamos Hipotecarios',
        subtitle: 'Realice su proyecto inmobiliario',
        desc: 'Conviértase en propietario en las mejores condiciones. Ya sea para la compra de una vivienda habitual, segunda residencia o una inversión de alquiler, nuestros expertos negocian la mejor tasa para usted.',
        advantagesTitle: 'Ventajas de nuestros préstamos hipotecarios',
        advantages: [
          'Tasas fijas y variables extremadamente competitivas',
          'Financiación flexible adaptada a sus ingresos',
          'Apoyo completo desde la simulación hasta la firma ante notario',
          'Negociación de las mejores condiciones de crédito con nuestros socios',
          'Soluciones de seguros para prestatarios a medida',
          'Estudio gratuito de su expediente de financiación',
        ],
        featuresTitle: 'Características de los préstamos hipotecarios',
        features: [
          { title: 'Importes elevados', desc: 'Financiación de hasta el 80% del valor del inmueble' },
          { title: 'Todos los proyectos', desc: 'Compra de obra nueva, usada, construcción o reformas' },
          { title: 'Seguro', desc: 'Garantías adecuadas para asegurar su inversión' },
          { title: 'Tasas ventajosas', desc: 'Las mejores tasas negociadas del mercado' },
        ]
      },
      rachat: {
        title: 'Reagrupación de Crédito',
        subtitle: 'Reagrupe todos sus créditos',
        desc: 'La reagrupación de crédito (o consolidación de deudas) consiste en reunir todos sus préstamos activos (hipotecarios, de consumo, tarjetas) en un único crédito. Reduce sus cuotas mensuales y simplifica la gestión de su presupuesto.',
        advantagesTitle: 'Ventajas de la reagrupación de crédito',
        advantages: [
          'Reduzca sus cuotas mensuales globales hasta un 60%',
          'Simplifique su gestión con un solo adeudo al mes',
          'Benefíciese de una tasa de interés única y potencialmente mejor',
          'Una sola cuota mensual fija y predecible',
          'Posibilidad de incluir dinero extra para un nuevo proyecto',
          'Asesoramiento personalizado y estudio gratuito',
        ]
      },
      travaux: {
        title: 'Crédito Reformas',
        subtitle: 'Financie sus proyectos de reforma',
        desc: 'Un crédito para reformas le permite financiar cualquier proyecto de acondicionamiento, reforma o decoración de su hogar. Ya sean obras de ahorro energético o de ampliación, tenemos la solución.',
        advantagesTitle: 'Ventajas del crédito reformas',
        advantages: [
          'Financiación del 100% del presupuesto de sus obras',
          'Tasas preferenciales para obras de eficiencia energética',
          'Plazo de devolución flexible adaptado a su presupuesto',
          'Desembolso de fondos sencillo y rápido presentando presupuestos',
          'No se requiere aportación personal',
          'Estudio gratuito y asesoramiento de expertos',
        ]
      },
      assurances: {
        title: 'Nuestros Seguros',
        subtitle: 'Proteja lo que más le importa',
        desc: 'Para asegurar sus financiaciones y proteger a sus seres queridos, le ofrecemos una amplia gama de pólizas de seguro adaptadas a sus necesidades y al mejor precio del mercado.',
        advantagesTitle: 'Nuestras soluciones de protección',
        advantages: [
          'Seguro de amortización: asegure su préstamo ante imprevistos (fallecimiento, invalidez)',
          'Seguro de hogar: proteja su patrimonio inmobiliario y sus pertenencias',
          'Seguro de salud: coberturas adaptadas para cubrir sus gastos médicos',
          'Tarifas negociadas entre las más bajas del mercado',
          'Cambio de seguro de préstamo sencillo y rápido',
          'Asesores dedicados para la gestión de sus contratos',
        ]
      },
      about: {
        title: 'Sobre Nosotros',
        subtitle: 'Su socio financiero de confianza',
        desc: 'Desde nuestra creación, nuestra misión ha sido acompañarle en la realización de todos sus proyectos aportándole las soluciones de financiación más adecuadas y eficientes.',
        advantagesTitle: 'Por qué confiar en nosotros',
        advantages: [
          'Más de 19 años de experiencia en el sector del crédito y la intermediación',
          'Un equipo de 43 asesores expertos a su servicio',
          'Total independencia de los bancos para defender sus intereses',
          'Más de 10.000 clientes satisfechos y apoyados con éxito',
          'Valores de transparencia, cercanía y eficacia',
          'Un servicio de atención al cliente de calidad premiado por su capacidad de respuesta',
        ]
      }
    },
  },
  pt: {
    nav: {
      home: 'Início',
      personal: 'Empréstimos Pessoais',
      mortgage: 'Crédito Habitação',
      rachat: 'Consolidação de Crédito',
      travaux: 'Crédito Obras',
      assurances: 'Seguros',
      about: 'Sobre Nós',
      apply: 'Fazer um pedido de empréstimo',
    },
    hero: {
      welcome: 'Bem-vindo à EVENTIA FINANCE',
      subtitle: 'O seu especialista em financiamento',
      description:
        'Descubra os nossos serviços de financiamento com um acompanhamento especializado e personalizado.',
      cta1: 'Fazer um pedido',
      cta2: 'Simular o meu empréstimo',
      legal:
        'Um crédito compromete-o e deve ser reembolsado. Verifique a sua capacidade de reembolso antes de se comprometer.',
    },
    services: {
      title: 'Os Nossos Serviços de Financiamento',
      subtitle: 'Soluções adaptadas a todas as suas necessidades de financiamento',
      personal: {
        title: 'Empréstimos Pessoais',
        desc: 'Financie os seus projetos pessoais com condições vantajosas',
      },
      mortgage: {
        title: 'Crédito Habitação',
        desc: 'Concretize o seu projeto imobiliário com a nossa experiência',
      },
      consolidation: {
        title: 'Consolidação de Crédito',
        desc: 'Simplifique os seus reembolsos e reduza as suas prestações mensais',
      },
      travaux: {
        title: 'Crédito Obras',
        desc: 'Financie os seus projetos de renovação e melhoramento da sua casa',
      },
      assurances: {
        title: 'Seguros',
        desc: 'Proteja os seus familiares e os seus bens com as nossas soluções à medida',
      },
      discover: 'Descobrir',
    },
    why: {
      title: 'Porquê Escolher a EVENTIA FINANCE',
      security: {
        title: 'Segurança & Confiança',
        desc: 'Instituição autorizada com mais de 19 anos de experiência em financiamento',
      },
      speed: {
        title: 'Rapidez & Eficácia',
        desc: 'Resposta em 48h e disponibilização rápida dos seus fundos',
      },
      support: {
        title: 'Acompanhamento Especializado',
        desc: '43 consultores especializados para o guiar em cada etapa',
      },
    },
    simulator: {
      title: 'Simulador de empréstimo',
      subtitle: 'Calcule o seu financiamento em poucos passos',
      params: 'Parâmetros do empréstimo',
      loanType: 'Tipo de empréstimo',
      selectType: 'Selecionar um tipo',
      amount: 'Montante',
      duration: 'Duração',
      months: 'meses',
      years: 'anos',
      results: 'Resultados',
      monthly: 'Prestação mensal estimada',
      totalCost: 'Custo total do crédito',
      totalAmount: 'Montante total a pagar',
      rate: 'Taxa indicativa (TAEG)',
      resultNote:
        'Simulação indicativa, sem valor contratual. Contacte-nos para uma proposta personalizada.',
      cta: 'Fazer o meu pedido',
      types: {
        personal: 'Empréstimo pessoal',
        mortgage: 'Crédito habitação',
        consolidation: 'Consolidação de crédito',
        auto: 'Crédito automóvel',
      },
    },
    testimonials: {
      title: 'Depoimentos de clientes',
      subtitle: 'O que dizem os nossos clientes',
      reviews: 'avaliações',
      loanType: 'Tipo de empréstimo',
      amount: 'Montante',
      prev: 'Anterior',
      next: 'Seguinte',
      items: [
        {
          name: 'Maria Dubois',
          city: 'Paris',
          text: 'Excelente serviço, rápido e profissional. Recomendo a EVENTIA FINANCE!',
          type: 'Empréstimo pessoal',
          amount: '25 000 €',
        },
        {
          name: 'Pierre Martin',
          city: 'Lyon',
          text: 'Graças à EVENTIA FINANCE, obtive o meu crédito habitação com as melhores condições.',
          type: 'Crédito habitação',
          amount: '250 000 €',
        },
        {
          name: 'Sophie Leroux',
          city: 'Marselha',
          text: 'Perfeito para consolidar os meus créditos, muito mais simples agora!',
          type: 'Consolidação de crédito',
          amount: '45 000 €',
        },
      ],
    },
    guarantees: {
      title: 'As nossas garantias',
      satisfied: 'Clientes satisfeitos',
      response: 'Prazo médio de resposta',
      rating: 'Avaliação média',
    },
    cta: {
      title: 'Pronto para Financiar o seu Projeto?',
      subtitle: 'Os nossos especialistas acompanham-no no seu projeto de financiamento',
      button: 'Iniciar o meu pedido',
    },
    form: {
      pageTitle: 'Pedido de Crédito',
      pageSubtitle:
        'Complete o seu pedido de financiamento em poucos minutos. A nossa equipa de especialistas contactará rapidamente para estudar o seu projeto e propor as melhores soluções adaptadas ao seu perfil.',
      formTitle: 'Formulário de Pedido de Crédito',
      personalInfo: 'Informações Pessoais',
      firstName: 'Primeiro Nome',
      lastName: 'Apelido',
      email: 'Endereço de e-mail',
      confirmEmail: 'Confirmar endereço de e-mail',
      phone: 'Telefone',
      project: 'O seu Projeto de Financiamento',
      creditType: 'Tipo de crédito',
      chooseType: 'Escolha o tipo de crédito',
      currency: 'Moeda',
      chooseCurrency: 'Escolha a moeda',
      currencies: {
        EUR: 'Euro (€)',
        USD: 'Dólar americano ($)',
        GBP: 'Libra esterlina (£)',
        CHF: 'Franco suíço (CHF)',
        CAD: 'Dólar canadiano (C$)',
        AUD: 'Dólar australiano (A$)',
        JPY: 'Iene japonês (¥)',
        CNY: 'Yuan chinês (元)',
        INR: 'Rúpia indiana (₹)',
        BRL: 'Real brasileiro (R$)',
      },
      desiredAmount: 'Montante pretendido',
      desiredDuration: 'Duração pretendida (en meses)',
      situation: 'A sua Situação Financeira',
      profession: 'Situação profissional',
      selectSituation: 'Selecione a sua situação',
      professions: {
        employee: 'Empregado(a)',
        selfEmployed: 'Trabalhador(a) independente',
        civilServant: 'Funcionário(a) público',
        retired: 'Reformado(a)',
        other: 'Outro',
      },
      income: 'Rendimentos mensais líquidos',
      message: 'Mensagem complementar',
      submit: 'Enviar o meu pedido de crédito',
      required: '* Campos obrigatórios',
      trust: 'Porquê confiar em nós?',
      secure: 'Dados seguros',
      secureDesc: 'As suas informações estão protegidas e são tratadas com total confidencialidade',
      fast: 'Resposta rápida',
      fastDesc: 'Contactamos de volta no prazo máximo de 24 horas',
      advisors: 'Consultores disponíveis',
      advisorsDesc: '43 especialistas ao seu serviço de segunda a sexta-feira',
      advantages: 'As Nossas Vantagens',
      advantagesList: [
        '19 anos de experiência',
        '43 consultores especializados',
        'Taxas competitivas',
        'Acompanhamento personalizado',
      ],
      question: 'Tem alguma dúvida?',
      questionDesc: 'Não hesite em contactar-nos',
      hours: 'De segunda a sexta-feira, das 9h às 18h',
      success: 'O seu pedido foi enviado com sucesso! Um consultor contactará no prazo de 24 horas.',
      emailMismatch: 'Os endereços de e-mail não coincidem.',
    },
    footer: {
      tagline: 'O seu parceiro de confiança para todos os seus projetos de financiamento.',
      navigation: 'Navegação',
      services: 'Serviços',
      legalInfo: 'Informações da Empresa',
      legal: {
        form: 'Forma jurídica: SARL',
        capital: 'Capital: 17 000 000 €',
        siren: 'SIREN: 494 831 191',
        siret: 'SIRET: 494 831 191 00056',
        tva: 'IVA: FR56 494831191',
        rcs: 'RCS Paris n.º 424 264 281',
        rne: 'RNE: 19/03/2007',
        staff: 'Pessoal: 43 colaboradores',
      },
      address: '55 Place Charles de Gaulle, 59500 Douai, França',
      rights: 'Todos os direitos reservados.',
      privacyPolicy: 'Política de Privacidade',
      legalNotices: 'Avisos Legais',
      warning:
        'Um crédito compromete-o e deve ser reembolsado. Verifique a sua capacidade de reembolso antes de se comprometer.',
    },
    whatsapp: 'Falar no WhatsApp',
    pages: {
      personal: {
        title: 'Empréstimos Pessoais',
        subtitle: 'Financie todos os seus projetos pessoais',
        desc: 'O empréstimo pessoal é um crédito ao consumo sem necessidade de justificar a utilização dos fundos. Dispõe livremente do montante emprestado para financiar todos os seus projetos de vida (automóvel, viagens, eventos, equipamentos).',
        advantagesTitle: 'Vantagens dos empréstimos pessoais',
        advantages: [
          'Utilização livre dos fundos sem necessidade de justificação',
          'Resposta rápida e análise do processo em 48h',
          'Taxas competitivas e mensalidades fixas',
          'Sem necessidade de garantias ou fiadores',
          'Acompanhamento personalizado ao longo do empréstimo',
          'Análise gratuita e sem compromisso do seu processo',
        ],
      },
      mortgage: {
        title: 'Crédito Habitação',
        subtitle: 'Realize o seu projeto imobiliário',
        desc: 'Torne-se proprietário com as melhores condições. Quer seja para a compra de habitação própria permanente, secundária ou para investimento imobiliário, os nossos especialistas negociam a melhor taxa para si.',
        advantagesTitle: 'Vantagens do nosso crédito habitação',
        advantages: [
          'Taxas fixas e variáveis extremamente competitivas',
          'Financiamento flexível adaptado aos seus rendimentos',
          'Acompanhamento total desde a simulação até à assinatura no cartório',
          'Negociação das melhores condições de crédito junto dos nossos parceiros',
          'Soluções de seguro de vida e habitação à sua medida',
          'Análise gratuita do seu processo de financiamento',
        ],
        featuresTitle: 'Características do crédito habitação',
        features: [
          { title: 'Montantes elevados', desc: 'Financiamento até 80% do valor do imóvel' },
          { title: 'Todos os projetos', desc: 'Compra de casa nova, usada, construção ou obras' },
          { title: 'Seguro', desc: 'Garantias adaptadas para proteger o seu investimento' },
          { title: 'Taxas vantajosas', desc: 'As melhores taxas negociadas do mercado' },
        ],
      },
      rachat: {
        title: 'Consolidação de Crédito',
        subtitle: 'Reúna todos os seus créditos',
        desc: 'A consolidação de créditos consiste em juntar todos os seus empréstimos em curso (habitação, consumo, cartões de crédito) num único crédito. Reduz assim as suas mensalidades globais e simplifica a gestão do seu orçamento.',
        advantagesTitle: 'Vantagens da consolidação de crédito',
        advantages: [
          'Reduza as suas mensalidades globais até 60%',
          'Simplifique a sua gestão com apenas uma prestação mensal',
          'Beneficie de uma taxa de juro única e potencialmente mais baixa',
          'Uma única mensalidade fixa e previsível',
          'Possibilidade de incluir liquidez adicional para um novo projeto',
          'Aconselhamento personalizado e análise gratuita',
        ],
      },
      travaux: {
        title: 'Crédito Obras',
        subtitle: 'Financie os seus projetos de renovação',
        desc: 'O crédito obras permite-lhe financiar todos os seus projetos de remodelação, decoração ou melhoramento do seu lar. Quer se trate de obras de eficiência energética ou de ampliação, temos a solução certa.',
        advantagesTitle: 'Vantagens do crédito obras',
        advantages: [
          'Financiamento de 100% dos seus orçamentos de obras',
          'Taxas preferenciais para melhorias de eficiência energética',
          'Prazo de reembolso flexível e adaptado ao seu orçamento',
          'Libertação de fundos simples e rápida mediante apresentação de orçamentos',
          'Sem necessidade de entrada inicial',
          'Análise e conselhos de especialistas gratuitos',
        ],
      },
      assurances: {
        title: 'Os Nossos Seguros',
        subtitle: 'Proteja o que mais importa para si',
        desc: 'Para proteger os seus financiamentos e assegurar o bem-estar dos seus familiares, propomos uma gama completa de contratos de seguro adaptados às suas necessidades e com os melhores preços do mercado.',
        advantagesTitle: 'As nossas soluções de proteção',
        advantages: [
          'Seguro de vida/crédito: garanta o pagamento do seu empréstimo em caso de imprevisto (morte, invalidez)',
          'Seguro multirriscos habitação: proteja o seu património imobiliário e recheio',
          'Seguro de saúde: coberturas adaptadas para apoiar as suas despesas médicas',
          'Tarifas negociadas entre as mais baixas do mercado',
          'Transferência de seguro de crédito simples e rápida',
          'Consultores dedicados para a gestão dos seus contratos',
        ],
      },
      about: {
        title: 'Sobre Nós',
        subtitle: 'O seu parceiro financeiro de confiança',
        desc: 'Desde a nossa fundação, a nossa missão tem sido acompanhá-lo na realização de todos os seus projetos, proporcionando as soluções de financiamento mais adequadas e eficientes.',
        advantagesTitle: 'Porquê confiar em nós',
        advantages: [
          'Mais de 19 anos de experiência na área do crédito e intermediação financeira',
          'Uma equipa de 43 consultores especializados e prontos a escutar',
          'Total independência face aos bancos para defender os seus interesses',
          'Mais de 10 000 clientes satisfeitos e apoiados com sucesso',
          'Valores de transparência, proximidade e eficácia',
          'Um serviço de apoio ao cliente de qualidade premiado pela sua rapidez de resposta',
        ],
      },
    },
  },
} as const

export type Translations = (typeof translations)['fr']
